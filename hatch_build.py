...
class CustomBuildHook(BuildHookInterface):
    ...
    def initialize(self, version, build_data):
        ...
        if os.environ.get("CADQUERY_WEB_VIEWER_SKIP_FRONTEND_BUILD") == "true":
            self.app.display_info(
                "Frontend build disabled via CADQUERY_WEB_VIEWER_SKIP_FRONTEND_BUILD"
            )
            return

        # Get the project root directory
        project_root = Path(self.root)
        frontend_dir = project_root / "cadquery_web_viewer" / "frontend"
    ...
    def _add_frontend_to_build_data(self, build_data, frontend_dir):
        ...
        build_data["force_include"][frontend_dir_str] = "cadquery_web_viewer/frontend"
